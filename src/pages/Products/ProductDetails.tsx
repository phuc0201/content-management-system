...
export default function ProductDetails() {
  ...
  const valueByKey = (key: keyof ProductFormState) => {
    ...
    if (!productResult || isCreateMode) return "";
    ...
    const fallbackMap: Record<keyof ProductFormState, string> = {
      ...
      salePrice: productResult?.data?.salePrice ? String(productResult?.data?.salePrice) : "",
      category: String(productResult?.data?.category ?? productResult?.data?.categoryId ?? ""),
      summary: String(productResult?.data?.summary ?? ""),
      description: String(productResult?.data?.description ?? ""),
    };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      salePrice: valueByKey("salePrice") ? Number(valueByKey("salePrice")) : null,
      category,
    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="space-y-4 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
          <div>
            <p className="mb-2 text-sm text-gray-700 dark:text-gray-200">Tên sản phẩm</p>
            <Input value={valueByKey("name")} onChange={(e) => setField("name", e.target.value)} />
          </div>

          <div>
            <p className="mb-2 text-sm text-gray-700 dark:text-gray-200">Slug</p>
            <Input value={valueByKey("slug")} onChange={(e) => setField("slug", e.target.value)} />
          </div>

          <div>
            <p className="mb-2 text-sm text-gray-700 dark:text-gray-200">Danh mục</p>
            <Select
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="mb-2 text-sm text-gray-700 dark:text-gray-200">Giá gốc</p>
              <Input
                type="number"
            ...
            <div>
              <p className="mb-2 text-sm text-gray-700 dark:text-gray-200">Giá giảm</p>
              <Input
                type="number"
          ...
          <div>
            <p className="mb-2 text-sm text-gray-700 dark:text-gray-200">Tóm tắt</p>
            <TextArea
        ...
        <div className="flex flex-col gap-4 justify-between rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
          <div className="flex-1 flex flex-col">
            <p className="text-sm text-gray-700 dark:text-gray-200">Gallery ảnh</p>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        <p className="mb-3 text-sm text-gray-700 dark:text-gray-200">Mô tả chi tiết</p>
        <RichTextEditor
